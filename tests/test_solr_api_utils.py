...
class TestSolrApiUtils(unittest.TestCase):
    ...
    def test_most_recent_configsets(self, mock_get_configsets):
        mock_get_configsets.return_value = [
            "tul_cob-catalog-9",
            "tul_cob-catalog-10",
            "tul_cob-web-2",
            "_default",
    ...
    @patch("tulflow.solr_api_utils.SolrApiUtils.get_alias_collections")
    def test_remove_collection_from_alias(
        self,
        mock_get_alias_collections,
        mock_create_or_modify_alias_and_set_collections,
    ):
        alias = "funcake-oai"
        mock_get_alias_collections.return_value = ["funcake-villanova", "funcake-temple"]
        self.solrcloud.remove_collection_from_alias("funcake-villanova", alias)
        mock_create_or_modify_alias_and_set_collections.assert_called_with(
            alias=alias,
            collections=["funcake-temple"],
        )
    ...
    @patch("tulflow.solr_api_utils.SolrApiUtils.create_or_modify_alias_and_set_collections")
    @patch("tulflow.solr_api_utils.SolrApiUtils.get_alias_collections")
    def test_add_collection_to_alias(
        self,
        mock_get_alias_collections,
        mock_create_or_modify_alias_and_set_collections,
    ):
        ...
        mock_create_or_modify_alias_and_set_collections.assert_called_with(
            alias=alias,
            collections=["funcake-temple", "funcake-villanova"],
        )
    ...
    def test_remove_and_recreate_collection_from_alias(
        self,
        mock_add_collection_to_alias,
        mock_create_collection,
        mock_delete_collection,
        mock_remove_collection_from_alias,
    ):
    ...
    def test_create_collection(self, mock_get_from_solr_api):
        ...
        replicationFactor = 5
        maxShardsPerNode = 2
        self.solrcloud.create_collection(
            collection=collection,
            configset=configset,
            numShards=numShards,
